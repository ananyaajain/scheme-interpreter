...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        new_args = rest.map(lambda x: scheme_eval(x, env))
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        body = procedure.body 
        en = procedure.env
        new = en.make_child_frame(formals, args)
        #print("DEBUG:", eval_all(body, new))
        return eval_all(body, new) 
        ...
        formals = procedure.formals
        body = procedure.body 
        new = env.make_child_frame(formals, args)
        return eval_all(body, new)
        # END PROBLEM 11
    else:
...
def optimize_tail_calls(unoptimized_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM EC
        while result == Unevaluated(expr, env): 
