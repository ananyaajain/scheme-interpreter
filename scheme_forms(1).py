...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    lst = expressions.rest
    return LambdaProcedure(formals, lst, env)
    # END PROBLEM 7
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    # BEGIN PROBLEM 14
    while not (bindings == nil):
        symbol = bindings.first
        rest = bindings.rest
        names = Pair(symbol.first, names)
        validate_formals(names)  
        validate_form(symbol.rest, 1, 1)
        vals = Pair(scheme_eval(symbol.rest.first, env), vals)
